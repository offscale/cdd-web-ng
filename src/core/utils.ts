...
// --- String Manipulation Utilities --- (No changes here)
...
// --- TypeScript Type Resolution --- (No changes here)
...
// --- General & OpenAPI Helpers --- (Changes are in this section)
    ...
    for (const [path, pathItem] of Object.entries(swaggerPaths)) {
        for (const method of methods) {
            ...
            if (operation) {
                ...
                // **CRITICAL FIX**: Separate body parameters from others to correctly normalize Swagger 2.0.
                const nonBodyParams = allParams.filter(p => p.in !== 'body');
                const bodyParam = allParams.find(p => p.in === 'body');

                // Normalize path, query, header, etc. parameters into a single format.
                ...
                paths.push({
                    ...
                    tags: operation.tags || [],
                    parameters, // This now correctly contains ONLY non-body parameters
...
// --- DI Token Name Generators --- (No changes here)
